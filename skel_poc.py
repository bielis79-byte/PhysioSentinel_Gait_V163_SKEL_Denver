...
def _v163_official_skin_data(mesh_seq):
    """Obtiene sólo topología/pesos oficiales del sexo activo, sin persistir el PKL."""
    cached=st.session_state.get("v163_official_skin_data")
    gender=str((mesh_seq or {}).get("skel_gender","")).strip().lower()
    if gender not in ("male","female"):
        try:
            gender,_=_patient_gender_145()
        except Exception:
            gender=None
    if isinstance(cached,dict) and cached.get("gender")==gender:
        return cached
    if gender not in ("male","female"):
        return None
    model_name=f"skel_{gender}.pkl"
    model_path=Path(tempfile.gettempdir())/"physiosentinel_skel_models_cache_v1_1"/model_name
    try:
        if not model_path.exists() or model_path.stat().st_size<1024:
            raw,_,_=_load_private_skel_bundle_auto()
            if raw is None:
                return None
            model_path.parent.mkdir(parents=True,exist_ok=True)
            with zipfile.ZipFile(io.BytesIO(raw)) as z:
                match=next(n for n in z.namelist() if n.lower().endswith(model_name))
                tmp=model_path.with_suffix(".pkl.part")
                with z.open(match) as src,open(tmp,"wb") as dst:
                    shutil.copyfileobj(src,dst)
                tmp.replace(model_path)
        import pickle
        with open(model_path,"rb") as pf:
            pdata=pickle.load(pf,encoding="latin1")
        compact={"gender":gender,
                 "skin_template_f":pdata.get("skin_template_f"),
                 "skin_weights":pdata.get("skin_weights"),
                 "bone_names":pdata.get("bone_names")}
        st.session_state["v163_official_skin_data"]=compact
        return compact
    except Exception:
        return None
...
def _plot_skel_mesh_walker(mesh_seq):
    """V163 · visor Denver y piel del pie neutralizada con pesos SKEL oficiales."""
    try:
        ...
        norm={_simple_name(n):i for i,n in enumerate(names)}
        # V163: la corrección visual del offset del pie usa el skinning oficial
        # específico male/female. No modifica q(t), joints ni el NPZ científico.
        Vviz=V.copy(); Jviz=J.copy(); _skin_qc_v163=None
        _official=_v163_official_skin_data(mesh_seq)
        if _official is not None:
            try:
                from visible_human_atlas import neutralize_skel_skin_with_official_weights
                Vviz,_skin_qc_v163=neutralize_skel_skin_with_official_weights(
                    V,F,J,names,_official
                )
            except Exception as _skin_exc:
                st.warning(f"V163: piel SKEL original conservada; corrección oficial bloqueada por QC: {_skin_exc}")
        ...
        if _skin_qc_v163 and _skin_qc_v163.get("pass"):
            _sx=str(_official.get("gender",mesh_seq.get("skel_gender",""))).lower()
            st.success(
                f"V163 · Piel del pie alineada con pesos SKEL oficiales ({_sx}); "
                "q(t) y patrón individual de marcha permanecen intactos."
            )
        elif _official is None:
            st.info("V163 · Para neutralizar también la piel del pie, debe estar disponible el PKL SKEL oficial del sexo registrado.")
        ...
        if not _ham_status.get("ready"):
            ...
            _cache_key=("v163",int(len(_Sv)),int(_Sv.shape[1]),float(np.nanmean(_Jv[0])),_vh_cache_sha,tuple(np.round(_axis_map_v161.reshape(-1),3)))
            ...
            if _DC is not None:
                ...
                if _aqc.get("pass"):
                    st.success(
                        "V162 · QC anatómico superado: sacro posterior, hallux bilateral medial, "
                        "neutro del tobillo y continuidad temporal validados antes del render."
                    )
                    with st.expander("V162 · Detalle del control anatómico",expanded=False):
                        st.json(_aqc)
                _DCjson={
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    if not runtime:
        ...
        try:
            ...
            # V163: conservar en memoria únicamente los datos necesarios para
            # neutralizar la piel del pie con el rig oficial del sexo activo.
            st.session_state["v163_official_skin_data"]={
                "gender":gender,
                "skin_template_f":pdata.get("skin_template_f"),
                "skin_weights":pdata.get("skin_weights"),
                "bone_names":pdata.get("bone_names"),
